...
describe("seedSessions", () => {
  ...
  const seeded = seedSessions({ app: salesApp, schema, versionId: "v1", count: 6, now: 1_000_000_000, seed: 42 });

  it("flags every seeded event and call", () => {
    expect(seeded.events.length).toBeGreaterThan(0);
    expect(seeded.events.every((e) => e.seeded && e.sessionId.startsWith("seeded-"))).toBe(true);
  ...
  it("produces the buried date-range friction on the recorded v1", () => {
    const metrics = computeMetrics({ versionId: "v1", schema, events: seeded.events, calls: seeded.calls });
    expect(metrics.sessions).toMatchObject({ total: 6, seeded: 6, live: 0 });
    const findings = findFriction({ app: salesApp, schema, metrics });
    expect(findings.some((f) => f.kind === "buried-control" && f.componentIds[0] === "date-range")).toBe(true);
  });
});
