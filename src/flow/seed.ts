    ...
    for (let s = 0; s < input.count; s++) {
        ...
        for (const component of visible) {
            ...
            push(
                ...
                'component_view',
                Math.max(0, row - 1) * 1_100 + random() * 300
            );
        }
        ...
        for (const control of visible) {
            ...
            if (capability?.kind !== 'state' || capability.stateType === 'text')
                continue;
            ...
            const driven = visible.filter(
                (c) =>
                    app.capability(c.capability)?.kind === 'data' &&
                    app
                        .capability(c.capability)
                        ?.dependsOn.includes(capability.id)
            );
            const target =
                driven.find(
                    (c) =>
                        c.primitive === 'line-chart' ||
                        c.primitive === 'bar-chart'
                ) ?? driven[0];
            ...
            for (let i = 0; i < changes; i++) {
                ...
                if (target)
                    push(target.id, target.capability, 'component_click', t);
                t += 3_000 + random() * 4_000;
            }
        }

        // Search, then read the collection it drives.
        for (const search of visible.filter(
            (c) => c.primitive === 'search-field'
        )) {
            if (random() > 0.6) continue;
            push(search.id, search.capability, 'value_change', t);
            const table = visible.find((c) =>
                app
                    .capability(c.capability)
                    ?.dependsOn.includes(search.capability)
            );
        ...
        const action = visible.find(
            (c) =>
                app.capability(c.capability)?.kind === 'action' &&
                c.id.includes('export')
        );
        if (action) {
