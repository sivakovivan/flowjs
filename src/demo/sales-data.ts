...
const FIRST = ["Ava", "Liam", "Maya", "Noah", "Iris", "Omar", "Zoe", "Ezra", "Lena", "Kai", "Ruth", "Theo", "Nia", "Jude", "Sana", "Felix"];
const LAST = ["Okafor", "Lindqvist", "Tanaka", "Moreau", "Reyes", "Kowalski", "Haddad", "Brennan", "Castillo", "Nakamura", "Petrov", "Adeyemi"];
const COMPANY = ["Northwind", "Bluefin Labs", "Cobalt & Co", "Juniper Supply", "Kestrel Retail", "Lumen Goods", "Meridian", "Orchard Market", "Pinecrest", "Quarry Studio"];
...
const RANGE_DAYS: Record<DateRange, number> = { "7d": 7, "30d": 30, "90d": 90, "12m": 365 };

function buckets(range: DateRange): Array<{ label: string; start: number; end: number }> {
  const days = RANGE_DAYS[range];
  const size = range === "7d" || range === "30d" ? 1 : range === "90d" ? 7 : 30;
  ...
  return Array.from({ length: count }, (_, i) => {
    ...
    return { label: format.format(end - DAY), start: end - size * DAY, end };
  });
}
...
function dailyValue(day: number, base: number, amplitude: number): number {
  ...
  return Math.max(0, base * (1 + weekly + seasonal + trend + noise) + amplitude * noise);
}

function series(range: DateRange, base: number, amplitude: number, round: (n: number) => number): TimeseriesData {
  const sumRange = (start: number, end: number) => {
    let total = 0;
    for (let day = start; day < end; day += DAY) total += dailyValue(day, base, amplitude);
    return total;
  };
  const points = buckets(range).map((b) => ({ label: b.label, value: round(sumRange(b.start, b.end)) }));
  const total = round(points.reduce((sum, p) => sum + p.value, 0));
  const days = RANGE_DAYS[range];
  const previous = sumRange(TODAY - 2 * days * DAY + DAY, TODAY - days * DAY + DAY);
  return { points, total, change: previous ? Math.round(((total - previous) / previous) * 1000) / 1000 : null };
}
...
export async function getCustomers(query: string): Promise<CollectionData> {
  ...
  const rows = CUSTOMERS.filter(
    (c) => !q || c.name.toLowerCase().includes(q) || c.company.toLowerCase().includes(q) || c.id.toLowerCase() === q,
  );
  return { rows: rows.slice(0, 12), total: rows.length };
}

export async function getTransactions(range: DateRange): Promise<CollectionData> {
...
export async function exportReport(format: "csv" | "pdf", range: DateRange) {
  ...
  if (format === "pdf") {
    return { format, filename: `sales-${range}.pdf`, message: `PDF report for ${total} transactions is ready.` };
  }
  const header = "id,customer,amount,date,status";
  const lines = rows.map((r) => [r.id, `"${r.customer}"`, r.amount, r.date, r.status].join(","));
  return {
    format,
...
export async function refundTransaction(transactionId: string) {
  ...
  if (!transaction) throw new Error(`Transaction ${transactionId} does not exist.`);
  if (transaction.status === "failed") throw new Error(`${transactionId} failed and cannot be refunded.`);
  if (refunded.has(transactionId)) throw new Error(`${transactionId} was already refunded.`);
  refunded.add(transactionId);
  return { transactionId, message: `Refunded ${transactionId} ($${transaction.amount.toFixed(2)}).` };
}
