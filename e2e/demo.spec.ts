...
async function setMutationRate(page: Page, rate: number) {
  const saved = page.waitForResponse((r) => r.url().endsWith("/api/flow/settings") && r.ok());
...
test("capabilities in, adaptive interface out", async ({ page }) => {
  await page.goto("/");
  await expect(page.getByRole("heading", { name: "No dashboard layout was written." })).toBeVisible();
  await page.getByRole("button", { name: "Generate dashboard" }).click();
  await expect(versionBadge(page)).toHaveText("v1");
  await expect(page.locator(".stage").getByText("Recorded response")).toBeVisible();
  ...
  for (const range of ["7d", "90d", "30d"]) {
    ...
    await page.locator('[data-component="revenue-chart"] .app-chart').click();
  }
  await page.getByRole("button", { name: "Export CSV" }).click();
  await expect(page.getByText(/Exported \d+ of \d+ transactions/)).toBeVisible();

  // Actions still work: refund the first paid transaction.
  const table = page.locator('[data-component="transactions-table"]');
  await table.locator("tr", { has: page.locator(".app-pill--paid") }).first().click();
  ...
  await expect(page.locator(".metrics-table tr", { hasText: "date-range" })).toContainText("3");
  ...
  await expect(evidence.getByText(/Based on 7 sessions \(1 live, 6 seeded\)/)).toBeVisible();
  ...
  await expect(page.locator('[data-component="date-range"] .change-tag')).toHaveText("Moved, Resized, New control");
  ...
  await expect(segmented.getByRole("radio", { name: "7 days" })).toHaveAttribute("aria-checked", "true");
...
test("mutation rate 0 never applies automatically", async ({ page }) => {
  ...
  await expect(page.getByText("Rate 0: never applies changes automatically")).toBeVisible();
  await page.getByRole("button", { name: "Optimize now" }).click();
  await expect(page.getByText("Mutation rate is 0, so nothing applies automatically.")).toBeVisible();
  await expect(page.getByRole("button", { name: "Apply change" })).toBeVisible();
  await page.waitForTimeout(4_000);
  await expect(versionBadge(page)).toHaveText("v1");
});

test("a high mutation rate applies automatically and branches from the active version", async ({ page }) => {
  await page.goto("/");
