...
export function findFriction(input: {
    app: FlowApp;
    schema: UISchema;
    metrics: Metrics;
}): Finding[] {
    ...
    const latencyOf = (capabilityId: string) =>
        metrics.latency.find((l) => l.capabilityId === capabilityId);
    const label = (m: ComponentMetrics) =>
        `${app.capability(m.capabilityId)?.label ?? m.capabilityId} (${m.componentId})`;
    const findings: Finding[] = [];

    for (const m of metrics.components) {
        const capability = app.capability(m.capabilityId);
        if (!capability || !m.visible) continue;
        const size = schema.components.find(
            (c) => c.id === m.componentId
        )?.size;
        ...
        const hardToSee =
            row > 0 &&
            ((m.avgFirstViewMs ?? 0) >= HEURISTICS.buried.belowFoldMs ||
                row >= lastRow / 2);
        if (
            ...
            findings.push({
                ...
                classification: 'ui',
                severity: round(
                    m.usageRate * Math.min(1, m.avgDiscoveryMs / 10_000)
                ),
                ...
                evidence: [
                    ...
                    `rendered in row ${row + 1} of ${lastRow + 1}` +
                        (m.avgFirstViewMs !== null
                            ? `, scrolled into view after ${secs(m.avgFirstViewMs)}`
                            : ''),
                    describeLatency(latency),
                ],
                suggestion:
                    'Move it closer to the top or beside the content it controls.',
            });
        }
        ...
        if (
            ...
            ((m.valueChangesPerUsingSession ?? 0) >=
                HEURISTICS.poorPrimitive.minChangesPerSession ||
            ...
            findings.push({
                ...
                severity: round(
                    Math.min(
                        1,
                        m.repeatRate + (m.valueChangesPerUsingSession ?? 0) / 10
                    )
                ),
                ...
                suggestion:
                    'Swap the dropdown for a segmented control so every option is one click away.',
            });
        }
        ...
        if (
            ...
            metrics.totalInteractions >=
                HEURISTICS.oversized.minTotalInteractions &&
            ...
            findings.push({
                ...
                severity: round(
                    0.5 *
                        (1 - m.interactionShare / HEURISTICS.oversized.maxShare)
                ),
                ...
                suggestion:
                    'Resize it smaller or move it lower to free space for high-use components.',
            });
        }
        ...
        if (
            ...
            const slow =
                m.retryLatencyMs !== null
                    ? m.retryLatencyMs >= SLOW_LATENCY_MS
                    : (latency?.slow ?? false);
            findings.push({
                ...
                evidence: [
                    ...
                    ...(m.retryLatencyMs !== null
                        ? [
                              `backend median ${Math.round(m.retryLatencyMs)}ms in sessions that retried`,
                          ]
                        : []),
                    describeLatency(latency),
                    ...(latency?.traceIds.length
                        ? [`Sentry traces: ${latency.traceIds.join(', ')}`]
                        : []),
                ],
                suggestion: slow
    ...
    for (const transition of metrics.transitions) {
        ...
        const share =
            from.followedBy.find((f) => f.componentId === to.componentId)
                ?.share ?? 0;
        const distance = Math.abs(
            (rows.get(from.componentId) ?? 0) - (rows.get(to.componentId) ?? 0)
        );
        if (
        ...
        findings.push({
            ...
            classification: 'ui',
            severity: round(
                share * Math.min(1, distance / 3) * (related ? 1 : 0.7)
            ),
