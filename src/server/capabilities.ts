...
export async function runCapability(
  ...
  if (!capability || capability.kind !== kind) {
    throw new CapabilityInputError(`Unknown ${kind} capability "${capabilityId}".`);
  }
  ...
  return Sentry.startSpan(
    {
      ...
      attributes: {
        ...
        ...(context.replayId ? { "flow.replay_id": context.replayId } : {}),
      },
    },
    async (span) => {
      const traceId = Sentry.isEnabled() ? span.spanContext().traceId : null;
      const started = performance.now();
      let ok = true;
      let error: string | null = null;
      try {
        if (kind === "data") {
          const value = await app.fetchData(capabilityId, context.state);
          if (capability.kind !== "data") throw new Error("unreachable");
          return { value: parseDataOutput(capability.output, value), traceId };
        }
        return { value: await app.executeAction(capabilityId, context.input, context.state), traceId };
      } catch (caught) {
        ok = false;
        error = caught instanceof Error ? caught.message : String(caught);
        span.setStatus({ code: 2, message: error });
        Sentry.logger.error("flow.capability failed", { capability: capabilityId, kind, error });
        throw caught instanceof CapabilityInputError ? caught : new CapabilityInputError(error);
      } finally {
        ...
        if (ok) Sentry.logger.info("flow.capability", { capability: capabilityId, kind, latencyMs });
      }
    },
  );
}
