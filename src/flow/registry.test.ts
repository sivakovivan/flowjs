...
import { CapabilityInputError, createFlowApp, FlowConfigError } from "./registry";
...
describe("createFlowApp", () => {
  it("registers data, actions and state as serializable descriptors", () => {
    ...
    expect(JSON.parse(JSON.stringify(app.capabilities))).toEqual(app.capabilities);
  });
  ...
  it("builds a dependency graph from state to dependents", () => {
    ...
    expect(graph.dependentsOf("dateRange")).toEqual(["revenue", "transactions", "exportReport"]);
  ...
  it("rejects invalid registrations with every problem listed", () => {
    ...
    config.actions.refundTransaction.inputs = { transactionId: { type: "string", from: "revenue" } };
    try {
  ...
  it("passes only declared, valid state to data functions", async () => {
    const app = fixtureApp();
    await expect(app.fetchData("revenue", { dateRange: "7d", customerQuery: "x" })).resolves.toEqual({
      range: "7d",
    });
    await expect(app.fetchData("revenue", {})).resolves.toEqual({ range: "30d" });
    await expect(app.fetchData("revenue", { dateRange: "5y" })).rejects.toBeInstanceOf(CapabilityInputError);
  });

  it("validates action inputs against the declared contract", async () => {
    const app = fixtureApp();
    await expect(app.executeAction("exportReport", { format: "csv" }, {})).resolves.toEqual({
      format: "csv",
      dateRange: "30d",
    });
    await expect(app.executeAction("exportReport", { format: "xls" }, {})).rejects.toThrow(/format/);
    await expect(app.executeAction("exportReport", { format: "csv", extra: 1 }, {})).rejects.toThrow();
    await expect(app.executeAction("revenue", {}, {})).rejects.toThrow(/Unknown action/);
  });
});
...
describe("compatiblePrimitives", () => {
  it("maps each capability shape to its allowed primitives", () => {
    ...
    expect(of("revenue")).toEqual(["metric-card", "line-chart", "bar-chart"]);
    expect(of("transactions")).toEqual(["table", "list"]);
    expect(of("dateRange")).toEqual(["dropdown", "segmented-control", "button-group"]);
  ...
  it("restricts long option lists to dropdowns", () => {
    ...
    expect(compatiblePrimitives(app.capability("dateRange")!)).toEqual(["dropdown"]);
  });
});
