...
describe('findFriction', () => {
    it('detects the buried, dropdown-driven date range and its separation from revenue', () => {
        const findings = findingsFor(dateRangeFriction(), [
            call('revenue', 90),
        ]);
        ...
        expect(separated.evidence.join()).toMatch(
            /related in the capability graph/
        );
    });

    it('does not call a visible, top-row control buried just because it is used late', () => {
        const buried = findingsFor(dateRangeFriction()).filter(
            (f) => f.kind === 'buried-control'
        );
    ...
    it('classifies retries on a slow action as backend performance', () => {
        const slow = [
            call('exportReport', 2900, { traceId: 'abc' }),
            call('exportReport', 3100),
        ];
        const finding = findingsFor(exportRetries(), slow).find(
            (f) => f.kind === 'high-retry-action'
        )!;
    ...
    it('uses latency from the retrying sessions, not the global median', () => {
        const calls = [
            call('exportReport', 2900, {
                sessionId: 's',
                componentId: 'export',
                traceId: 'slow-trace',
            }),
            ...Array.from({ length: 5 }, () =>
                call('exportReport', 100, {
                    sessionId: 'other',
                    componentId: 'export',
                })
            ),
        ];
        const finding = findingsFor(exportRetries(), calls).find(
            (f) => f.kind === 'high-retry-action'
        )!;
        expect(finding.classification).toBe('performance');
        expect(finding.evidence.join()).toMatch(
            /median 2900ms in sessions that retried/
        );
    });

    it('classifies retries on a fast action as interface friction', () => {
        const fast = [call('exportReport', 120), call('exportReport', 140)];
        const finding = findingsFor(exportRetries(), fast).find(
            (f) => f.kind === 'high-retry-action'
        )!;
    ...
    it('flags large components that get almost no interaction', () => {
        ...
        const events = Array.from({ length: 16 }, (_, i) =>
            session(`s${i}`, [['date-range', 'value_change', 1]], {
                start: i * 100_000,
            })
        ).flat();
        const findings = findingsFor(events, [], schema);
        const oversized = findings
            .filter((f) => f.kind === 'oversized-low-value')
            .map((f) => f.componentIds[0]);
        expect(oversized).toEqual(
            expect.arrayContaining(['revenue-chart', 'transactions-table'])
        );
    });

    it('stays quiet without enough evidence', () => {
        expect(
            findingsFor(session('s', [['date-range', 'value_change', 1]]))
        ).toEqual([]);
    });

    it('reports a separated pair once, in its dominant direction', () => {
        const separated = findingsFor(dateRangeFriction()).filter(
            (f) => f.kind === 'separated-related'
        );
        const pairs = separated.map((f) =>
            [...f.componentIds].sort().join('+')
        );
        expect(new Set(pairs).size).toBe(pairs.length);
        expect(separated[0].componentIds).toEqual([
            'date-range',
            'revenue-chart',
        ]);
    });
    ...
    it('sorts findings by severity', () => {
        const severities = findingsFor(dateRangeFriction()).map(
            (f) => f.severity
        );
