...
function withComponent(
    id: string,
    patch: Partial<UISchema['components'][number]>
): UISchema {
...
describe('validateSchema', () => {
    ...
    it('normalizes order to a dense index', () => {
        const schema = fixtureSchema();
        schema.components.forEach(
            (c, i) => (c.order = (schema.components.length - i) * 10)
        );
        ...
        expect(result.schema.components.map((c) => c.order)).toEqual([
            0, 1, 2, 3, 4, 5,
        ]);
    });
    ...
    it('rejects malformed output', () => {
        const result = validateSchema(
            { components: [{ id: 'Bad Id', capability: 'revenue' }] },
            app
        );
    ...
    it('rejects unknown capabilities', () => {
        const result = validateSchema(
            withComponent('export', { capability: 'deleteEverything' }),
            app
        );
        expect(result).toMatchObject({ ok: false });
        expect(!result.ok && result.errors.join()).toMatch(
            /unknown capability "deleteEverything"/
        );
    });

    it('rejects incompatible primitives', () => {
        const result = validateSchema(
            withComponent('date-range', { primitive: 'line-chart' }),
            app
        );
        expect(!result.ok && result.errors.join()).toMatch(
            /cannot render state "dateRange" as "line-chart"/
        );
    });
    ...
    it('requires every capability to be represented', () => {
        const schema = fixtureSchema();
        schema.components = schema.components.filter(
            (c) => c.capability !== 'customerQuery'
        );
        const result = validateSchema(schema, app);
        expect(!result.ok && result.errors.join()).toMatch(
            /"customerQuery" has no component/
        );
    });

    it('requires required capabilities to stay visible', () => {
        const hiddenDate = validateSchema(
            withComponent('date-range', { visible: false }),
            app
        );
        expect(!hiddenDate.ok && hiddenDate.errors.join()).toMatch(
            /Required capability "dateRange"/
        );
        // Unread, non-required state may be hidden.
        expect(
            validateSchema(
                withComponent('customer-search', { visible: false }),
                app
            ).ok
        ).toBe(true);
    });
...
describe('layoutRows', () => {
    ...
    it('skips hidden components', () => {
        const rows = layoutRows(
            withComponent('transactions-table', { visible: false })
        );
...
describe('diffSchemas', () => {
    it('reports what moved, resized, swapped and changed visibility', () => {
        ...
        const after = withComponent('date-range', {
            order: -1,
            primitive: 'segmented-control',
        });
        const changes = diffSchemas(
            before,
            withComponent('customer-search', { visible: false, size: 'small' })
        );
        expect(changes).toEqual({ 'customer-search': ['resized', 'hidden'] });
        expect(diffSchemas(before, after)['date-range']).toEqual([
            'moved',
            'swapped',
        ]);
    });
    ...
    it('marks only the moved component, not the ones that shift to make room', () => {
        ...
        expect(diffSchemas(fixtureSchema(), after)).toEqual({
            'date-range': ['moved'],
        });
        // And the reverse (undo) highlights the same component.
        expect(diffSchemas(after, fixtureSchema())).toEqual({
            'date-range': ['moved'],
        });
    });
});
