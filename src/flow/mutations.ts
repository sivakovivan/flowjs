...
import { normalizeOrder, validateSchema, type UIComponent, type UISchema } from "./schema";
...
export const MUTATION_TYPES = ["MOVE", "REORDER", "RESIZE", "SWAP_VARIANT", "SHOW", "HIDE"] as const;
...
export const MutationSchema = z.discriminatedUnion("type", [
  ...
  z.strictObject({ type: z.literal("REORDER"), element: z.string(), index: z.number().int() }),
  z.strictObject({ type: z.literal("RESIZE"), element: z.string(), size: z.enum(SIZES) }),
  z.strictObject({ type: z.literal("SWAP_VARIANT"), element: z.string(), variant: z.enum(PRIMITIVES) }),
...
function applyOne(
  ...
  if (index === -1) return { error: `${describe(mutation)}: unknown component "${mutation.element}".` };
  const element = ordered[index];
  const replace = (patch: Partial<UIComponent>) =>
    ordered.map((c) => (c.id === element.id ? { ...c, ...patch } : c));
  const renumber = (list: UIComponent[]) => list.map((c, order) => ({ ...c, order }));

  switch (mutation.type) {
    case "MOVE": {
      if (mutation.target === element.id) return { error: `${describe(mutation)}: cannot move beside itself.` };
      const target = ordered.find((c) => c.id === mutation.target);
      if (!target) return { error: `${describe(mutation)}: unknown target "${mutation.target}".` };
      if (!target.visible) return { error: `${describe(mutation)}: target "${target.id}" is hidden.` };
      const rest = ordered.filter((c) => c.id !== element.id);
      const targetIndex = rest.findIndex((c) => c.id === target.id);
      rest.splice(mutation.position === "before" ? targetIndex : targetIndex + 1, 0, element);
      if (rest.every((c, i) => c.id === ordered[i].id)) {
        return { error: `${describe(mutation)}: already ${mutation.position} "${target.id}".` };
      }
      return { components: renumber(rest) };
    }
    case "REORDER": {
      if (mutation.index < 0 || mutation.index >= ordered.length) {
        return { error: `${describe(mutation)}: index ${mutation.index} is out of range.` };
      }
      if (mutation.index === index) return { error: `${describe(mutation)}: already at index ${index}.` };
      const rest = ordered.filter((c) => c.id !== element.id);
      rest.splice(mutation.index, 0, element);
      return { components: renumber(rest) };
    }
    case "RESIZE": {
      if (element.size === mutation.size) return { error: `${describe(mutation)}: already ${mutation.size}.` };
    ...
    case "SWAP_VARIANT": {
      ...
      if (element.primitive === mutation.variant) {
        return { error: `${describe(mutation)}: already a ${mutation.variant}.` };
      }
      return { components: replace({ primitive: mutation.variant }) };
    }
    case "SHOW": {
      if (element.visible) return { error: `${describe(mutation)}: already visible.` };
      return { components: replace({ visible: true }) };
    }
    case "HIDE": {
      if (!element.visible) return { error: `${describe(mutation)}: already hidden.` };
      const capability = app.capability(element.capability);
      const otherVisible = ordered.some(
        (c) => c.id !== element.id && c.capability === element.capability && c.visible,
      );
 ...
 */
export function applyMutations(schema: UISchema, proposal: unknown[], app: FlowApp): MutationResult {
  if (proposal.length === 0) return { ok: false, errors: ["Proposal contains no mutations."] };
  if (proposal.length > MAX_MUTATIONS_PER_PROPOSAL) {
    return { ok: false, errors: [`Proposal exceeds ${MAX_MUTATIONS_PER_PROPOSAL} mutations.`] };
  }
  ...
  for (const [i, raw] of proposal.entries()) {
    ...
    if (!parsed.success) {
      errors.push(`Mutation ${i + 1} is malformed: ${parsed.error.issues.map((issue) => issue.message).join("; ")}`);
      continue;
    }
