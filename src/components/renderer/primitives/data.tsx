...
export function MetricCard({ component, capability }: DataProps) {
    const { data, loading, error } = useCapabilityData<TimeseriesData>(
        capability.id,
        component.id
    );
    ...
    return (
        ...
        >
            {data ? (
                <>
                    <strong className="app-metric__value">
                        {formatValue(data.total, unitOf(capability))}
                    </strong>
                    {change && (
                        <span
                            className={`app-metric__change ${data.change! >= 0 ? 'is-up' : 'is-down'}`}
                        >
...
function useSelectable(capability: DataCapability) {
    ...
    return [...capabilities.values()].some(
        (c) =>
            c.kind === 'action' &&
            Object.values(c.inputs).some(
                (input) =>
                    input.type === 'string' && input.from === capability.id
            )
    );
}

export function Table({ component, capability }: DataProps) {
    const { data, loading, error } = useCapabilityData<CollectionData>(
        capability.id,
        component.id
    );
    ...
    if (data.rows.length === 0)
        return <Status loading={false} error={null} empty />;
