...
export type RunStatus = "rejected" | "pending" | "auto" | "applied" | "no-change" | "stale";
...
export class FlowStore {
  ...
  syncApplication(app: FlowApp): ApplicationRecord {
    return this.transaction(() => {
      ...
      this.db
        ...
        .run(app.id, app.name, app.context, app.mutationRate, JSON.stringify(app.theme), now);
      this.db.prepare("DELETE FROM capabilities WHERE application_id = ?").run(app.id);
      ...
      for (const capability of app.capabilities) {
        const { id, kind, description, label, dependsOn, required, ...shape } = capability;
        insert.run(id, app.id, kind, description, JSON.stringify(shape), JSON.stringify({ label, dependsOn, required }), now);
      }
  ...
  setMutationRate(applicationId: string, rate: number): void {
    if (!(rate >= 0 && rate <= 1)) throw new RangeError("mutationRate must be between 0 and 1.");
    this.db.prepare("UPDATE applications SET mutation_rate = ? WHERE id = ?").run(rate, applicationId);
  }
  ...
  createVersion(input: {
    ...
    return this.transaction(() => {
      if (input.parentVersionId && !this.getVersion(input.applicationId, input.parentVersionId)) {
        throw new VersionError(`Parent version "${input.parentVersionId}" does not exist.`);
      }
      const { next } = this.db
        .prepare("SELECT COALESCE(MAX(number), 0) + 1 AS next FROM ui_versions WHERE application_id = ?")
  ...
  activateVersion(applicationId: string, versionId: string, cause: ActivationCause): VersionRecord {
    return this.transaction(() => {
      const version = this.getVersion(applicationId, versionId);
      if (!version) throw new VersionError(`Version "${versionId}" does not exist.`);
      this.db.prepare("UPDATE applications SET active_version_id = ? WHERE id = ?").run(versionId, applicationId);
      this.db
        .prepare("INSERT INTO version_activations (application_id, version_id, cause, created_at) VALUES (?, ?, ?, ?)")
  ...
  undo(applicationId: string): VersionRecord {
    return this.transaction(() => {
      ...
      if (!active.parentVersionId) throw new VersionError(`${active.id} has no parent to undo to.`);
      return this.activateVersion(applicationId, active.parentVersionId, "undo");
    });
  }

  getVersion(applicationId: string, id: string): VersionRecord | null {
    const row = this.db
      .prepare("SELECT * FROM ui_versions WHERE application_id = ? AND id = ?")
  ...
  getActiveVersion(applicationId: string): VersionRecord | null {
    ...
    return app?.activeVersionId ? this.getVersion(applicationId, app.activeVersionId) : null;
  }

  listVersions(applicationId: string): VersionRecord[] {
    return (
      this.db
        .prepare("SELECT * FROM ui_versions WHERE application_id = ? ORDER BY number DESC")
  ...
  listEvents(applicationId: string, versionId: string): TelemetryEvent[] {
    const rows = this.db
      .prepare("SELECT * FROM telemetry_events WHERE application_id = ? AND version_id = ? ORDER BY created_at, id")
  ...
  listCalls(applicationId: string, limit = 2000): CapabilityCall[] {
    const rows = this.db
      .prepare("SELECT * FROM capability_calls WHERE application_id = ? ORDER BY created_at DESC, id DESC LIMIT ?")
  ...
  clearSeeded(applicationId: string): void {
    this.transaction(() => {
      this.db.prepare("DELETE FROM telemetry_events WHERE application_id = ? AND seeded = 1").run(applicationId);
      this.db.prepare("DELETE FROM capability_calls WHERE application_id = ? AND seeded = 1").run(applicationId);
    });
  }

  // ── optimization runs ─────────────────────────────────────────────────────

  createRun(input: Omit<OptimizationRun, "id" | "createdAt" | "appliedVersionId">): OptimizationRun {
    const id = randomUUID();
    this.db
      .prepare(
  ...
  getRun(id: string): OptimizationRun | null {
    const row = this.db.prepare("SELECT * FROM optimization_runs WHERE id = ?").get(id) as Row | undefined;
    return row ? toRun(row) : null;
  }

  setRunStatus(id: string, status: RunStatus, appliedVersionId: string | null = null): void {
    this.db
      .prepare("UPDATE optimization_runs SET status = ?, accepted = ?, applied_version_id = ? WHERE id = ?")
