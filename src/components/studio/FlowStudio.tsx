...
export function FlowStudio() {
  ...
  const notify = useCallback((tone: Notice["tone"], text: string) => {
    ...
    setTimeout(() => setNotices((list) => list.filter((n) => n.id !== id)), 4_500);
  }, []);
  ...
  const transitionTo = useCallback(
    async (version: VersionRecord) => {
      ...
      if (previous) setChanges(diffSchemas(previous.schema, version.schema));
      setStudio((current) => (current ? { ...current, active: version } : current));
  ...
  async function generate() {
    ...
    try {
      ...
      await refreshState();
      if (provenance?.source === "recorded") notify("ok", "Generated v1 from a recorded AI response.");
  ...
  const apply = useCallback(
    async (target: OptimizationRun, mode: "auto" | "manual") => {
      ...
      try {
        ...
        notify("error", error instanceof Error ? error.message : "Could not apply the change.");
        setRun((current) => (current ? { ...current, status: "stale" } : current));
      } finally {
        setApplying(false);
      }
    },
  ...
  async function optimize() {
    ...
    try {
      ...
      notify("error", error instanceof Error ? error.message : "Optimization failed.");
    } finally {
      setOptimizing(false);
    }
  }
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => setCountdown((n) => (n === null ? null : n - 1)), 1_000);
  ...
  async function undo() {
    try {
      ...
      notify("error", error instanceof Error ? error.message : "Nothing to undo.");
    }
  }
  ...
  async function restore(versionId: string) {
    ...
    try {
      ...
      notify("error", error instanceof Error ? error.message : "Could not restore.");
    }
  }
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      api.setMutationRate(rate).then(
        ({ application }) => setStudio((s) => (s ? { ...s, application } : s)),
  ...
  async function seed() {
    ...
    try {
      ...
      notify("error", error instanceof Error ? error.message : "Could not seed sessions.");
    } finally {
      setBusy(false);
    }
  }
  ...
  const componentMetrics = useMemo(
    () => new Map((metrics?.versionId === activeId ? metrics.metrics.components : []).map((m) => [m.componentId, m])),
    [metrics, activeId],
  );
  ...
  if (loadError) {
    return (
      <main className="studio studio--error">
        <p role="alert">flow.js could not load its state: {loadError}</p>
      </main>
    );
  }
  if (!studio) {
      ...
      <main className="studio">
        <header className="bar">
          <div className="bar__brand">
            ...
            <span className="bar__app">{studio.application.name}</span>
          </div>

          {active && (
            <div className="bar__versions">
              <span className="version-badge" aria-label={`Active version ${active.id}`}>
                {active.id}
              </span>
              <button type="button" className="chrome-button" onClick={undo} disabled={!active.parentVersionId}>
                ↶ Undo
              </button>
        ...
        {!active ? (
          <GeneratePrompt studio={studio} generating={generating} error={generateError} onGenerate={generate} />
        ) : (
          <div className="workspace">
            <section className="stage" style={themeStyle(studio.application.theme)} aria-label={`${studio.application.name} dashboard`}>
              <div className="stage__meta">
                <p>
                  Generated interface, {active.id}: {active.reason}
                </p>
                {generatedProvenance && active.source === "generated" && (
                  <SourceBadge
              ...
              <RendererProvider
                ...
                notify={notify}
              >
                <Dashboard schema={active.schema} metrics={componentMetrics} changes={changes} showTelemetry={showTelemetry} />
              </RendererProvider>
            </section>

            <aside className="panel" aria-label="flow.js runtime">
              <div className="tabs" role="tablist">
                {(["telemetry", "evidence", "capabilities"] as const).map((name) => (
                  <button
                    key={name}
                    type="button"
                    role="tab"
                    ...
                    onClick={() => setTab(name)}
                  >
                    {name === "telemetry" ? "Telemetry" : name === "evidence" ? "Optimization" : "Capabilities"}
                  </button>
                ))}
              </div>
              <div className="panel__body" role="tabpanel">
                {tab === "telemetry" && (
                  <TelemetryPanel data={metrics} onSeed={seed} onClearSeeded={clearSeeded} busy={busy} />
                )}
                {tab === "evidence" && (
                  <EvidencePanel
                    run={run}
                    ...
                    onApply={() => run && apply(run, "manual")}
                    onCancelAuto={() => setCountdown(null)}
                  />
                )}
                {tab === "capabilities" && <CapabilitiesPanel studio={studio} />}
              </div>
              <p className="panel__foot">
                {studio.sentry.enabled ? "Sentry tracing and replay are on." : "Sentry is off (no DSN); latency is still measured locally."}
              </p>
            </aside>
          </div>
        )}
