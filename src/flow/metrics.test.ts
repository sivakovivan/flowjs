...
function metricsFor(
    events = dateRangeFriction(),
    calls = [call('revenue', 120)]
) {
...
describe('computeMetrics', () => {
    it('counts live and seeded sessions separately', () => {
        const seeded = session('seeded', [['export', 'component_click', 1]], {
            seeded: true,
        });
        expect(
            metricsFor([...dateRangeFriction(), ...seeded]).sessions
        ).toEqual({ total: 4, live: 3, seeded: 1 });
    });

    it('derives usage, discovery time, repeat rate and value changes', () => {
        const dateRange = metricsFor().components.find(
            (c) => c.componentId === 'date-range'
        )!;
    ...
    it('records interaction sequences and follow-ups', () => {
        ...
        expect(metrics.transitions[0]).toEqual({
            from: 'date-range',
            to: 'revenue-chart',
            count: 9,
        });
        const dateRange = metrics.components.find(
            (c) => c.componentId === 'date-range'
        )!;
        expect(dateRange.followedBy).toEqual([
            { componentId: 'revenue-chart', count: 9, share: 1 },
        ]);
    });
    ...
    it('computes error rate from completions and errors', () => {
        ...
        const exportMetrics = metricsFor(events).components.find(
    ...
    it('reports untouched components with zero usage', () => {
        const refund = metricsFor().components.find(
            (c) => c.componentId === 'refund'
        )!;
...
describe('retry latency', () => {
    it('measures backend latency only in sessions where the component was retried', () => {
        ...
        const once = session('fast', [['export', 'component_click', 1]], {
            start: 5_000_000,
        });
        const calls = [
            call('exportReport', 2800, {
                sessionId: 'slow',
                componentId: 'export',
            }),
            call('exportReport', 90, {
                sessionId: 'fast',
                componentId: 'export',
            }),
            call('exportReport', 110, {
                sessionId: 'fast',
                componentId: 'export',
            }),
            call('exportReport', 95, {
                sessionId: 'other',
                componentId: 'export',
            }),
        ];
        const exportMetrics = metricsFor(
            [...retried, ...once],
            calls
        ).components.find((c) => c.componentId === 'export')!;
        expect(exportMetrics.retryLatencyMs).toBe(2800);
        // Across all calls the median looks fast; only the tail shows it.
        expect(
            computeLatency(calls).find((l) => l.capabilityId === 'exportReport')
        ).toMatchObject({
            slow: false,
            slowTail: true,
        });
    ...
    it('is null when nothing was retried', () => {
        const exportMetrics = metricsFor(
            session('s', [['export', 'component_click', 1]])
        ).components.find((c) => c.componentId === 'export')!;
...
describe('computeLatency', () => {
    it('summarizes backend latency and flags slow capabilities', () => {
        ...
        const exportLatency = latency.find(
            (l) => l.capabilityId === 'exportReport'
        )!;
        expect(exportLatency).toMatchObject({
            calls: 3,
            p50Ms: 2900,
            p95Ms: 3100,
            slow: true,
        ...
        expect(latency.find((l) => l.capabilityId === 'revenue')?.slow).toBe(
            false
        );
    });
});
