    ...
    return {
        ...
        proposeOptimization: (brief) =>
            call(
                OPTIMIZATION_INSTRUCTIONS,
                brief,
                zodTextFormat(OptimizationOutput, 'flow_optimization')
            ),
    };
}
        ...
        async proposeOptimization(brief) {
            ...
            if (top?.classification === 'performance') {
                const diagnosis = recording.optimizations.find(
                    (candidate) =>
                        candidate.classification === 'performance' &&
                        candidate.mutations.length === 0
                );
                if (diagnosis) return structuredClone(diagnosis);
            }
            const schema: UISchema = {
                components: brief.currentSchema.map(
                    ({ row: _row, ...component }) => component
                ),
            };
            const match = recording.optimizations.find(
                (candidate) =>
                    ...
                    applyMutations(
                        schema,
                        candidate.mutations.map(toMutation),
                        app
                    ).ok
            );
            if (!match)
                throw new Error(
                    'No recorded proposal applies to the current version.'
                );
