...
export function HistoryMenu(props: {
  ...
  return (
    <div className="history" ref={ref} role="dialog" aria-label="UI history">
      <p className="history__title">UI history</p>
      <ol className="history__list">
        {props.versions.map((version) => {
          const active = version.id === props.activeId;
          return (
            <li key={version.id} className={active ? "is-active" : undefined}>
              <span className="history__id">{version.id}</span>
              <div className="history__body">
                <p className="history__reason">{version.reason}</p>
                <p className="history__meta">
                  {version.parentVersionId ? `From ${version.parentVersionId}` : "Generated from capabilities"}
                  {", "}
                  {time.format(version.createdAt)}
                  {version.aiSource === "recorded" && ", recorded AI response"}
                </p>
              </div>
              {active ? (
                <span className="history__current">Current</span>
              ) : (
                <button type="button" className="chrome-link" onClick={() => props.onRestore(version.id)}>
                  Restore
                </button>
              )}
            </li>
