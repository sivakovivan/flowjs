...
const generation: GeneratedSchemaOutput = {
    ...
    components: fixtureSchema().components.map((c) => ({
        ...c,
        rationale: 'fixture',
    })),
};
...
const promoteDateRange: OptimizationOutput = {
    ...
    explanation:
        'Move the date range beside the chart it filters and make every option one click.',
    mutations: [
        {
            type: 'MOVE',
            element: 'date-range',
...
const recording: Recording = {
    provenance: 'test',
    generation,
    optimizations: [promoteDateRange],
};
...
describe('generate', () => {
    ...
    it('replays the recorded response when the live call fails', async () => {
        ...
        expect(provenance?.fallbackReason).toMatch(
            /Live call failed: 503 upstream/
        );
    });
    ...
    it('never persists an invalid live schema', async () => {
        ...
        const { version, provenance } = await runtime(
            stub({ generateSchema: async () => invalid })
        ).generate();
        expect(provenance?.source).toBe('recorded');
        expect(provenance?.fallbackReason).toMatch(
            /failed validation.*launchMissiles/
        );
        expect(
            version.schema.components.every((c) => app.capability(c.capability))
        ).toBe(true);
        expect(store.listVersions(app.id)).toHaveLength(1);
    });

    it('falls back when the live output is malformed', async () => {
        const { provenance } = await runtime(
            stub({ generateSchema: async () => ({ nope: true }) })
        ).generate();
        expect(provenance?.source).toBe('recorded');
    });
    ...
    it('requires an API key unless recorded mode is explicitly forced', async () => {
        ...
        expect(forced.provenance).toMatchObject({
            source: 'recorded',
            fallbackReason: expect.stringMatching(/Recorded mode/),
        });
    ...
    it('creates no version when both live and recorded outputs are unusable', async () => {
        const rt = createRuntime({
            ...
            store,
            live: stub({
                generateSchema: async () => Promise.reject(new Error('down')),
            }),
...
describe('optimize and apply', () => {
    ...
    it('proposes, scores and awaits approval below the threshold', async () => {
        ...
        expect(analysis.sampleSize).toEqual({
            sessions: 3,
            liveSessions: 3,
            seededSessions: 0,
            interactions: 21,
        });
        expect(analysis.findings.map((f) => f.kind)).toContain(
            'buried-control'
        );
    ...
    it('applies an approved proposal as a new active version with evidence', async () => {
        ...
        expect(version.schema.components[0]).toMatchObject({
            id: 'date-range',
    ...
    it('records performance findings as no-change diagnostics', async () => {
        const performance = {
            ...structuredClone(promoteDateRange),
            classification: 'performance' as const,
            mutations: [],
        };
        const rt = runtime(
            stub({ proposeOptimization: async () => performance })
        );
        await rt.generate();
        recordFriction();
        const run = await rt.optimize();
    ...
    it('replays a recorded proposal when the live optimization call fails', async () => {
        ...
        expect(
            (run.analysis as OptimizationAnalysis).ai.fallbackReason
        ).toMatch(/timeout/);
...
describe('telemetry ingestion', () => {
    ...
    it('accepts events for real components and derives the capability from the schema', async () => {
        ...
        expect(
            store.listEvents(app.id, 'v1').map((e) => e.capabilityId)
        ).toEqual(['dateRange', 'dateRange']);
    });
    ...
    it('seeds flagged sessions that can be cleared without touching live data', async () => {
        ...
        expect(rt.analyze().metrics.sessions).toMatchObject({
            live: 1,
            seeded: 4,
        });
        rt.clearSeeded();
