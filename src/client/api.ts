...
import type { ApplicationRecord, OptimizationRun, VersionRecord } from "@/flow/store";

/* Typed client for the flow.js route handlers. */

export type ClientCapability = CapabilityDescriptor & { compatiblePrimitives: Primitive[] };
export type VersionSummary = Omit<VersionRecord, "schema" | "telemetrySnapshot">;
...
export type { AIProvenance, OptimizationAnalysis, OptimizationRun, VersionRecord };
...
export const api = {
  ...
  generate: () => request<{ version: VersionRecord; provenance: AIProvenance | null }>("POST", "/generate"),
  ...
  apply: (runId: string, mode: "auto" | "manual") =>
    request<{ version: VersionRecord; run: OptimizationRun }>("POST", `/optimize/${runId}/apply`, { mode }),
  undo: () => request<{ version: VersionRecord }>("POST", "/versions/undo"),
  restore: (versionId: string) => request<{ version: VersionRecord }>("POST", `/versions/${versionId}/restore`),
  setMutationRate: (mutationRate: number) =>
    request<{ application: ApplicationRecord }>("PATCH", "/settings", { mutationRate }),
  seed: (sessions = 6) => request<{ sessions: number }>("POST", "/telemetry/seed", { sessions }),
  clearSeeded: () => request<{ cleared: boolean }>("DELETE", "/telemetry/seed"),
  data: (capability: string, body: Record<string, unknown>) =>
    request<{ value: unknown; traceId: string | null }>("POST", `/data/${capability}`, body),
  action: (capability: string, body: Record<string, unknown>) =>
    request<{ value: unknown; traceId: string | null }>("POST", `/actions/${capability}`, body),
};
