...
export interface ActionDefinition {
  ...
  dependsOn?: string[];
  required?: boolean;
  execute: (input: Record<string, string>, context: ActionContext) => Promise<unknown> | unknown;
}
...
export interface FlowApp {
  ...
  executeAction(id: string, input: Record<string, unknown>, state: StateValues): Promise<unknown>;
}
...
function normalizeInput(definition: ActionInputDefinition): ActionInput {
  if (definition === "string") return { type: "string", from: null, description: null };
  if (Array.isArray(definition)) return { type: "enum", options: [...definition] };
  const object = definition as Exclude<ActionInputDefinition, readonly string[] | "string">;
  if (object.type === "enum") return { type: "enum", options: [...object.options] };
  return { type: "string", from: object.from ?? null, description: object.description ?? null };
}
...
export function createFlowApp(config: FlowAppConfig): FlowApp {
  ...
  const claim = (id: string) => {
    if (!ID_PATTERN.test(id)) problems.push(`"${id}" is not a valid capability id (use camelCase).`);
    if (ids.has(id)) problems.push(`Capability id "${id}" is registered more than once.`);
    ids.add(id);
  };
  ...
  for (const [id, state] of Object.entries(config.state)) {
    ...
    if (state.type !== "text" && !options.includes(state.default)) {
      problems.push(`State "${id}" default "${state.default}" is not one of its options.`);
    }
    descriptors.push({
      id,
      kind: "state",
  ...
  for (const [id, action] of Object.entries(config.actions)) {
    ...
    const inputs = Object.fromEntries(
      Object.entries(action.inputs ?? {}).map(([name, input]) => [name, normalizeInput(input)]),
    );
    descriptors.push({
      id,
      kind: "action",
  ...
  for (const descriptor of descriptors) {
    for (const dependency of descriptor.dependsOn) {
      if (!stateIds.includes(dependency)) {
        problems.push(`"${descriptor.id}" depends on "${dependency}", which is not a registered state.`);
      }
    }
    if (descriptor.kind === "action") {
      for (const [name, input] of Object.entries(descriptor.inputs)) {
        ...
        if (!source || source.kind !== "data" || source.output.kind !== "collection") {
          problems.push(
  ...
  const edges = descriptors.flatMap((d) => d.dependsOn.map((from) => ({ from, to: d.id })));
  for (const descriptor of descriptors) {
    if (descriptor.kind === "state" && config.state[descriptor.id].required === undefined) {
      descriptor.required = edges.some((edge) => edge.from === descriptor.id);
    }
  }

  const mutationRate = config.mutationRate ?? 0.5;
  if (!(mutationRate >= 0 && mutationRate <= 1)) {
    problems.push(`mutationRate must be between 0 and 1 (received ${mutationRate}).`);
  }
  ...
  const graph: CapabilityGraph = {
    edges,
    dependentsOf: (stateId) => edges.filter((e) => e.from === stateId).map((e) => e.to),
    related(a, b) {
      if (a === b) return true;
      const depsA = byId.get(a)?.dependsOn ?? [];
      const depsB = byId.get(b)?.dependsOn ?? [];
      return depsA.includes(b) || depsB.includes(a) || depsA.some((dep) => depsB.includes(dep));
    },
  };

  function resolveState(capability: CapabilityDescriptor, state: StateValues): StateValues {
    ...
    for (const dependency of capability.dependsOn) {
      ...
      if (descriptor.stateType !== "text" && !descriptor.options.includes(value)) {
        throw new CapabilityInputError(`"${value}" is not a valid value for state "${dependency}".`);
      }
      if (typeof value !== "string" || value.length > 200) {
        throw new CapabilityInputError(`Invalid value for state "${dependency}".`);
      }
  ...
  function inputSchema(capability: Extract<CapabilityDescriptor, { kind: "action" }>) {
  ...
  return {
    ...
    defaultState() {
      return Object.fromEntries(
        descriptors.flatMap((d) => (d.kind === "state" ? [[d.id, d.default]] : [])),
      );
    },
    async fetchData(id, state) {
      const capability = byId.get(id);
      if (!capability || capability.kind !== "data") {
        throw new CapabilityInputError(`Unknown data capability "${id}".`);
      }
      return config.data[id].fetch({ state: resolveState(capability, state) });
    },
    async executeAction(id, input, state) {
      const capability = byId.get(id);
      if (!capability || capability.kind !== "action") {
        throw new CapabilityInputError(`Unknown action capability "${id}".`);
      }
      ...
      return config.actions[id].execute(parsed.data, { state: resolveState(capability, state) });
    },
  };
}
