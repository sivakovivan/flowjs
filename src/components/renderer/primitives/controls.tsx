...
const optionLabel = (capability: StateCapability, option: string) => capability.optionLabels[option] ?? option;
...
export function Dropdown({ component, capability }: StateProps) {
  ...
  return (
    <select
      ...
      onPointerDown={() => tracker.track(component.id, "interaction_start")}
      onFocus={() => tracker.track(component.id, "interaction_start")}
      onChange={(event) => setStateValue(component.id, capability.id, event.target.value)}
    >
...
export function OptionButtons({ component, capability, segmented }: StateProps & { segmented: boolean }) {
  const { state, setStateValue } = useRenderer();
  const current = state[capability.id] ?? capability.default;
  return (
    <div className={segmented ? "app-segmented" : "app-button-group"} role="radiogroup" aria-label={capability.label}>
      {capability.options.map((option) => (
        <button
          key={option}
          type="button"
          ...
          onClick={() => option !== current && setStateValue(component.id, capability.id, option)}
        >
          {optionLabel(capability, option)}
        </button>
      ))}
...
function useAction(component: UIComponent, capability: ActionCapability) {
  ...
  const bound = Object.entries(capability.inputs).flatMap(([name, input]) =>
    input.type === "string" && input.from ? [[name, input.from] as const] : [],
  );
  const boundValues = Object.fromEntries(bound.map(([name, from]) => [name, selection[from] ?? null]));
  const missing = bound.filter(([name]) => !boundValues[name]).map(([, from]) => from);
  ...
  async function run(extra: Record<string, string> = {}) {
    ...
    setRunning(true);
    await runAction(capability.id, component.id, { ...(boundValues as Record<string, string>), ...extra });
    setRunning(false);
  }
...
function enumInput(capability: ActionCapability) {
  const entry = Object.entries(capability.inputs).find(([, input]) => input.type === "enum");
  return entry && entry[1].type === "enum" ? { name: entry[0], options: entry[1].options } : null;
}

export function ActionButton({ component, capability }: ActionProps) {
  const { run, running, missing, boundValues } = useAction(component, capability);
  ...
  return (
    <div className="app-action">
      ...
      >
        {running ? "Working…" : target ? `${capability.label} ${target}` : capability.label}
      </button>
      {missing.length > 0 && <p className="app-hint">Select a row in {missing.join(", ")} first.</p>}
    </div>
  );
}
...
export function ActionButtonGroup({ component, capability }: ActionProps) {
  ...
  const choice = enumInput(capability);
  if (!choice) return <ActionButton component={component} capability={capability} />;
  const verb = capability.label.split(" ")[0];
  return (
    <div className="app-button-group" role="group" aria-label={capability.label}>
