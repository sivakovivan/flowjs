...
function ScoreMeter({ analysis }: { analysis: OptimizationAnalysis }) {
  ...
  return (
    <div className="score">
      <div className="score__bar" role="img" aria-label={`Score ${score.score}, threshold ${decision.threshold ?? "disabled"}`}>
        <span className="score__fill" style={{ width: `${clamp(score.score)}%` }} />
        {decision.threshold !== null && (
          <span className="score__threshold" style={{ left: `${clamp(decision.threshold)}%` }} />
        )}
      </div>
...
export function EvidencePanel(props: {
  ...
  if (!run) {
    return (
      <div className="panel-empty">
        ...
        <p>Use the dashboard, then choose Optimize now. flow.js will explain what it found before changing anything.</p>
      </div>
    );
  }
  ...
  const relevantLatency = analysis.latency.filter((l) => l.calls > 0).sort((a, b) => b.p50Ms - a.p50Ms);
    ...
    <article className="evidence" aria-live="polite">
      ...
      <section className="evidence__section">
        ...
        <p className="evidence__sample">
          Based on {sampleSize.sessions} session{sampleSize.sessions === 1 ? "" : "s"} ({sampleSize.liveSessions} live
          {sampleSize.seededSessions > 0 && `, ${sampleSize.seededSessions} seeded`}) and {sampleSize.interactions}{" "}
          interactions. Confidence {ai.confidence.toFixed(2)}.
        </p>
      </section>
        ...
        <section className="evidence__section">
          ...
          <ul className="findings">
            {analysis.findings.slice(0, 4).map((finding) => (
              <li key={`${finding.kind}:${finding.componentIds.join()}`}>
                <p>
                  <span className={`dot dot--${finding.classification}`} aria-hidden />
                  {finding.title}
                </p>
                <p className="findings__evidence">{finding.evidence.join("; ")}</p>
              </li>
            ))}
          </ul>
        </section>
          ...
          <table className="latency">
            ...
            <tbody>
              {relevantLatency.map((l) => {
                ...
                const href = trace ? sentryLink(props.sentryOrg, "trace", trace) : null;
                return (
                    ...
                    <td>
                      {trace ? (
                        href ? (
                          ...
                          <code title={trace}>{trace.slice(0, 8)}</code>
                        )
                      ) : (
      ...
      <section className="evidence__section">
        ...
        {mutations.length === 0 ? (
          <p>No interface change. {ai.classification === "performance" && "Fix the backend before redesigning this control."}</p>
        ) : (
          <ol className="mutations">
            {mutations.map((mutation, i) => (
              <li key={i}>
                <code>{mutation.type}</code> {describeMutation(mutation)}
              </li>
            ))}
          </ol>
        )}
        {run.status === "rejected" && (
          <div className="rejected" role="alert">
            <p>flow.js rejected this proposal. Nothing was changed.</p>
            <ul>
      ...
      <footer className="evidence__actions">
        {run.status === "applied" && <p className="applied">Applied as {run.appliedVersionId}.</p>}
        {run.status === "stale" && <p className="muted">The dashboard changed since this proposal. Optimize again.</p>}
        {run.status === "auto" && props.countdown !== null && (
          <>
            <p>Applying automatically in {props.countdown}s…</p>
            <button type="button" className="chrome-button" onClick={props.onCancelAuto}>
              Cancel
            </button>
          </>
        )}
        {(run.status === "pending" || (run.status === "auto" && props.countdown === null)) && (
          <button type="button" className="chrome-button chrome-button--signal" onClick={props.onApply} disabled={props.applying}>
