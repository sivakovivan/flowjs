...
describe("decideAutoApply", () => {
  it("never auto-applies at rate 0, even for a perfect score", () => {
    expect(decideAutoApply(1, 0)).toEqual({ autoApply: false, threshold: null });
  });
...
describe("scoreProposal", () => {
  const base = {
    ...
    sessions: 3,
    interactions: 30,
    mutations: [{ type: "MOVE", element: "a", target: "b", position: "before" } as const],
    recentVersions: 0,
  };
  ...
  it("discounts thin evidence", () => {
    expect(scoreProposal({ ...base, sessions: 1, interactions: 10 }).evidence).toBe(0.333);
  });

  it("penalizes recent instability and caps disruption", () => {
    expect(scoreProposal({ ...base, recentVersions: 2 }).disruption).toBe(0.15);
    expect(scoreProposal({ ...base, recentVersions: 50 }).disruption).toBe(0.5);
  });

  it("clamps out-of-range model outputs", () => {
    const score = scoreProposal({ ...base, expectedBenefit: 7, confidence: -1 });
