...
interface RendererContextValue {
  ...
  state: StateValues;
  setStateValue: (componentId: string, stateId: string, value: string) => void;
  ...
  runAction: (capabilityId: string, componentId: string, input: Record<string, string>) => Promise<unknown>;
...
export function useRenderer(): RendererContextValue {
  ...
  if (!value) throw new Error("useRenderer must be used inside <RendererProvider>.");
  return value;
}
...
export function RendererProvider(props: {
  ...
  const capabilities = useMemo(() => new Map(props.capabilities.map((c) => [c.id, c])), [props.capabilities]);
  ...
  const dependencyState = useCallback(
    (capabilityId: string) => {
      ...
      return Object.fromEntries((capability?.dependsOn ?? []).map((id) => [id, state[id] ?? ""]));
    },
    [capabilities, state],
  );
  ...
  const runAction = useCallback(
    async (capabilityId: string, componentId: string, input: Record<string, string>) => {
      ...
      try {
        ...
        tracker.track(componentId, "interaction_complete", { latencyMs: Math.round(performance.now() - started) });
        const result = value as { message?: string; filename?: string; content?: string };
        if (result.content && result.filename) download(result.filename, result.content);
        ...
        tracker.track(componentId, "interaction_error", { latencyMs: Math.round(performance.now() - started) });
  ...
  const setStateValue = useCallback((componentId: string, stateId: string, value: string) => {
    tracker.track(componentId, "value_change", { state: stateId, value });
    setState((previous) => ({ ...previous, [stateId]: value }));
  }, []);

  const select = useCallback((collectionId: string, rowKey: string | null) => {
    setSelection((previous) => ({ ...previous, [collectionId]: rowKey }));
  }, []);
  ...
  const value = useMemo<RendererContextValue>(
    ...
    [versionId, capabilities, state, setStateValue, selection, select, dataVersion, fetchData, runAction, notify],
  );

  return <RendererContext.Provider value={value}>{props.children}</RendererContext.Provider>;
}

/** Load a data capability for the current state; shared across components bound to it. */
export function useCapabilityData<T>(capabilityId: string, componentId: string) {
  const { fetchData } = useRenderer();
  const [result, setResult] = useState<{ data: T | null; error: string | null; loading: boolean }>({
    data: null,
    error: null,
    loading: true,
  });
  ...
  useEffect(() => {
    ...
    fetchData(capabilityId, componentId).then(
      (data) => !cancelled && setResult({ data: data as T, error: null, loading: false }),
      (error: Error) => !cancelled && setResult({ data: null, error: error.message, loading: false }),
    );
    return () => {
      cancelled = true;
    };
