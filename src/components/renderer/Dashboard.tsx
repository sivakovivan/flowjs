...
const LAYOUT_TRANSITION = { type: "spring", stiffness: 170, damping: 24, mass: 0.9 } as const;
  ...
  return (
    ...
    >
      <motion.header layout="position" className="app-card__header">
        <h2 className={isControl ? "app-card__title app-card__title--control" : "app-card__title"}>
          {capability.label}
        </h2>
        {changes && (
          <span className="change-tag" role="status">
            {changes.map((change) => CHANGE_LABEL[change]).join(", ")}
          </span>
        )}
...
export function Dashboard(props: {
  ...
  const components = [...props.schema.components].filter((c) => c.visible).sort((a, b) => a.order - b.order);
