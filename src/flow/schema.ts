...
export const UIComponentSchema = z.strictObject({
    id: z
        .string()
        .regex(/^[a-z][a-z0-9-]{0,47}$/, 'component ids are kebab-case'),
    capability: z.string(),
...
export function validateSchema(input: unknown, app: FlowApp): ValidationResult {
    ...
    for (const component of parsed.data.components) {
        ...
        if (!capability) {
            errors.push(
                `Component "${component.id}" references unknown capability "${component.capability}".`
            );
            continue;
        }
    ...
    for (const capability of app.capabilities) {
        const bound = parsed.data.components.filter(
            (c) => c.capability === capability.id
        );
        if (bound.length === 0) {
            errors.push(`Capability "${capability.id}" has no component.`);
        } else if (capability.required && !bound.some((c) => c.visible)) {
            errors.push(
                `Required capability "${capability.id}" has no visible component.`
            );
        }
    }
    ...
    return errors.length > 0
        ? { ok: false, errors }
        : { ok: true, schema: normalizeOrder(parsed.data) };
}
...
export type ComponentChange =
    'moved' | 'resized' | 'swapped' | 'shown' | 'hidden';
...
function movedComponents(before: UISchema, after: UISchema): Set<string> {
    const oldIndex = new Map(
        normalizeOrder(before).components.map((c) => [c.id, c.order])
    );
    const sequence = normalizeOrder(after).components.filter((c) =>
        oldIndex.has(c.id)
    );
    ...
    for (let i = 0; i < sequence.length; i++) {
        for (let j = 0; j < i; j++) {
            if (
                oldIndex.get(sequence[j].id)! < oldIndex.get(sequence[i].id)! &&
                length[j] + 1 > length[i]
            ) {
                length[i] = length[j] + 1;
                previous[i] = j;
...
export function diffSchemas(
    before: UISchema,
    after: UISchema
): Record<string, ComponentChange[]> {
    const previous = new Map(
        normalizeOrder(before).components.map((c) => [c.id, c])
    );
