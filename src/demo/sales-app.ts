...
export const salesApp = createFlowApp({
  ...
  data: {
    ...
    customers: {
      ...
      output: {
        ...
        columns: [
          ...
          { key: "lifetimeValue", label: "Lifetime value", format: "currency" },
    ...
    transactions: {
      description: "Recent customer transactions in the selected date range",
      output: {
        kind: "collection",
        rowKey: "id",
        columns: [
  ...
  actions: {
    exportReport: {
      ...
      dependsOn: ["dateRange"],
      execute: (input, { state }) => exportReport(input.format as "csv" | "pdf", state.dateRange as DateRange),
    },
    refundTransaction: {
  ...
  state: {
    dateRange: {
      ...
      type: "date-range",
      options: DATE_RANGES,
      optionLabels: { "7d": "7 days", "30d": "30 days", "90d": "90 days", "12m": "12 months" },
      default: "30d",
    },
    customerQuery: {
