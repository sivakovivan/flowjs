...
describe('MOVE', () => {
    it('places the element before the target', () => {
        ...
        expect(schema.components.map((c) => c.order)).toEqual([
            0, 1, 2, 3, 4, 5,
        ]);
    });
    ...
    it('places the element after the target', () => {
        ...
        expect(ids(schema).slice(0, 3)).toEqual([
            'revenue-chart',
            'export',
            'customer-search',
        ]);
    ...
    it('rejects unknown targets, self-targets, hidden targets and no-ops', () => {
        ...
        const hidden = expectOk(
            apply([{ type: 'HIDE', element: 'customer-search' }])
        );
        expectRejected(
            apply(
                [
...
describe('REORDER', () => {
    it('moves the element to an absolute index', () => {
        const schema = expectOk(
            apply([{ type: 'REORDER', element: 'refund', index: 1 }])
        );
        expect(ids(schema).slice(0, 3)).toEqual([
            'revenue-chart',
            'refund',
    ...
    it('rejects out-of-range and no-op indexes', () => {
        expectRejected(
            apply([{ type: 'REORDER', element: 'refund', index: 6 }]),
            /out of range/
        );
        expectRejected(
            apply([{ type: 'REORDER', element: 'refund', index: -1 }]),
        ...
        expectRejected(
            apply([{ type: 'REORDER', element: 'refund', index: 5 }]),
            /already at index/
        );
    });
});
...
describe('RESIZE', () => {
    it('changes the size', () => {
        ...
        expect(
            schema.components.find((c) => c.id === 'transactions-table')?.size
        ).toBe('medium');
    });
    ...
    it('rejects unknown sizes and no-ops', () => {
        expectRejected(
            apply([
                { type: 'RESIZE', element: 'transactions-table', size: 'huge' },
            ]),
            /malformed/
        );
        expectRejected(
            apply([
                { type: 'RESIZE', element: 'transactions-table', size: 'full' },
            ]),
            /already full/
        );
    });
...
describe('SWAP_VARIANT', () => {
    it('swaps to a compatible primitive', () => {
        ...
        expect(
            schema.components.find((c) => c.id === 'date-range')?.primitive
...
describe('SHOW / HIDE', () => {
    it('hides and shows a non-required component', () => {
        ...
        expect(
            hidden.components.find((c) => c.id === 'customer-search')?.visible
        ).toBe(false);
        const shown = expectOk(
            apply([{ type: 'SHOW', element: 'customer-search' }], hidden)
        );
        expect(
            shown.components.find((c) => c.id === 'customer-search')?.visible
        ).toBe(true);
    });

    it('refuses to hide the only usable control for a required capability', () => {
        expectRejected(
            apply([{ type: 'HIDE', element: 'date-range' }]),
            /only usable control.*dateRange/
        );
        expectRejected(
            apply([{ type: 'HIDE', element: 'export' }]),
            /only usable control.*exportReport/
        );
    });
    ...
    it("allows hiding a required capability's duplicate control", () => {
        ...
        expect(
            apply([{ type: 'HIDE', element: 'date-range' }], schema).ok
        ).toBe(true);
    });

    it('rejects no-op visibility changes', () => {
        expectRejected(
            apply([{ type: 'SHOW', element: 'date-range' }]),
            /already visible/
        );
    });
});
...
describe('proposals', () => {
    ...
    it('rejects unknown elements, unknown mutation types, empty and oversized proposals', () => {
        expectRejected(
            apply([{ type: 'HIDE', element: 'ghost' }]),
            /unknown component "ghost"/
        );
        expectRejected(
            apply([{ type: 'DELETE', element: 'export' }]),
            /malformed/
        );
