...
describe('sales demo registry', () => {
    ...
    it('returns data matching each declared output contract', async () => {
        for (const capability of salesApp.capabilities) {
            ...
            const value = await salesApp.fetchData(
                capability.id,
                salesApp.defaultState()
            );
            expect(() =>
                parseDataOutput(capability.output, value)
            ).not.toThrow();
        }
    });

    it('filters customers by the search state', async () => {
        const result = (await salesApp.fetchData('customers', {
            customerQuery: 'northwind',
        })) as {
    ...
    it('refunds once and rejects a second refund', async () => {
        const { rows } = (await salesApp.fetchData('transactions', {
            dateRange: '12m',
        })) as {
    ...
    it('chains every recorded optimization as a valid, sequential improvement', () => {
        ...
        for (const recorded of salesRecording.optimizations) {
            ...
            const result = applyMutations(
                schema,
                proposal.mutations.map(toMutation),
                salesApp
            );
            if (!result.ok)
                throw new Error(
                    `${proposal.reason}: ${result.errors.join(' ')}`
                );
...
describe('recorded provider', () => {
    ...
    it('replays a no-change diagnosis when backend performance is the top finding', async () => {
        const output = (await provider.proposeOptimization(
            brief([finding('performance')])
        )) as OptimizationOutput;
