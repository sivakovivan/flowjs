...
export function GeneratePrompt(props: {
    ...
    const { studio } = props;
    const willUseRecording =
        studio.ai.forcedRecorded || !studio.ai.liveConfigured;
    return (
        <div className="generate">
            <div className="generate__intro">
                <h1>No dashboard layout was written.</h1>
                <p>
                    The developer registered {studio.capabilities.length}{' '}
                    capabilities for {studio.application.name}: data, actions
                    and state, each with a description and a contract. flow.js
                    asks OpenAI to turn them into a first interface, validates
                    the result, and saves it as v1.
                </p>
                <button
                    type="button"
                    className="chrome-button chrome-button--signal chrome-button--large"
                    ...
                    <pre>
                        <code>
                            {studio.registration.slice(
                                studio.registration.indexOf('export const')
                            )}
                        </code>
                    </pre>
                </figure>
